...
class MapViewController: UIViewController,MKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        coder.geocodeAddressString(area.name!) { (ps, error) in
            ...
            let annotationg = MKPointAnnotation()
            annotationg.title = self.area.name
            annotationg.subtitle = self.area.province
            
            if let loc = place?.location{
                annotationg.coordinate = loc.coordinate
                self.mapView.showAnnotations([annotationg], animated: true)
                self.mapView.selectAnnotation(annotationg, animated: true)
            }
